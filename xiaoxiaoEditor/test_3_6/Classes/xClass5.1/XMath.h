...
public:
    ...
    inline static float mix(float a, float b, float v)
    {
        return a+(b-a)*v;
    }
    inline static Vec2 mix(const Vec2& a, const Vec2& b, float v)
    {
        return Vec2(mix(a.x,b.x,v),mix(a.y,b.y,v));
    }
    inline static float mix2x(float a, float b, float c, float v)
    {
        const float n = 1-v;
        return a*n*n+2*b*n*v+c*v*v;
    }
    inline static Vec2 mix2x(const Vec2& a, const Vec2& b, const Vec2& c, float v)
    {
        return Vec2(mix2x(a.x, b.x, c.x, v),mix2x(a.y, b.y, c.y, v));
    }
    inline static float mix3x(float a, float b, float c, float d, float v)
    {
        const float n = 1-v;
        return a*n*n*n+3*b*n*n*v+3*c*n*v*v+d*v*v*v;
    }
    inline static Vec2 mix3x(const Vec2& a, const Vec2& b, const Vec2& c, const Vec2& d, float v)
    {
        return Vec2(mix3x(a.x, b.x, c.x, d.x, v),mix3x(a.y, b.y, c.y, d.y, v));
    }
    inline static float umix(float a, float b, float f)
    {
        if(a == f || a == b) return 0;
        return (f-a)/(b-a);
    }
    inline static Vec2 mixUV(const Vec2& A,const Vec2& U,const Vec2& V,const Vec2& uv)
    {
        //P=(1-u-v)A+uC+vB
    ...
    inline static Vec2 umixUV(const Vec2& A,const Vec2& U,const Vec2& V,const Vec2& P)
    {
    ...
    inline static float radianLength(float from, float to, bool isAbs = false)
    {
        if (isAbs) {
    ...
    inline static float rotationLength(float from, float to, bool isAbs = false)
    {
        if (isAbs) {
    ...
    inline static bool isInTriangle(const Vec2& A,const Vec2& B,const Vec2& C,const Vec2& P)
    {
        const Vec2 v0(C-A);
        const Vec2 v1(B-A);
        const Vec2 v2(P-A);
    ...
    inline static Vec2 beelineIntersectPointValue(const Vec2& P1, const Vec2& P2, const Vec2& Q1, const Vec2& Q2)
    {
        float x;
        float y;
        if (P1.y == P2.y)
        {
            if(Q1.y == Q2.y) return Vec2(-100,-100);
            y = P1.y;
            if(Q2.x == Q1.x)
            {
                x = Q1.x;
            ...
            return Vec2(umix(P1.x, P2.x, x),umix(Q1.y, Q2.y, y));
        }
        if (Q1.y == Q2.y)
        {
            if(P1.y == P2.y) return Vec2(-100,-100);
            y = Q1.y;
            if(P2.x == P1.x)
            {
                x = P1.x;
            ...
            return Vec2(umix(P1.y, P2.y, y),umix(Q1.x, Q2.x, x));
        }
        //
        if (P1.x == P2.x)
        {
            if(Q1.x == Q2.x) return Vec2(-100,-100);
            x = P1.x;
            if(Q2.y == Q1.y)
            {
                y = Q1.y;
        ...
        if (Q1.x == Q2.x)
        {
            if(P1.x == P2.x) return Vec2(-100,-100);
            x = Q1.x;
            if(P2.y == P1.y)
            {
                y = P1.y;
        ...
        return Vec2(umix(P1.x, P2.x, xx),umix(Q1.x, Q2.x, xx));
    }
    inline static void split(string src, const char* token, vector<std::string>& vect)
    {
        int nend=0;
        int nbegin=0;
        while(nend != -1)
...
private:
    inline static float mathPower(float x, float p) {
        if (x > 0) {
            return exp(log(x)*p);
        } else if (x < 0) {
            return -exp(log(-x)*p);
    ...
    inline static float mathAtan2(float dx, float dy) {
        float a;
        if (dx == 0) {
            a = M_PI_2;
        } else if (dx > 0) {
...
public:
     ...
     * ax + b = 0
     **/
    inline static vector<float> solve1xEquation(float A, float B) {
        if (abs(A) < 1e-10) {
            return vector<float>();
        }
        return {-B/A};
     ...
     * ax^2 + bx + c = 0
     **/
    inline static vector<float> solve2xEquation(float A, float B, float C) {
     ...
     * ax^3 + bx^2 + cx + d = 0
     **/
    inline static vector<float> solve3xEquation(float A, float B, float C, float D) {
        if (abs(A) < 1e-10) {
        ...
        if (quadraticSolve.empty() || ((quadraticSolve[0] != x) && (quadraticSolve[1] != x))) {
    ...
    inline static vector<float> solve4xEquation(float A, float B, float C, float D, float E) {
